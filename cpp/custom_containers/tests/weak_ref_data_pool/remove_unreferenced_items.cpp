...
TEST_F(weak_ref_data_pool, remove_unreferenced_items_one_by_one) {
  common::reset();
...

  {
    ...
    ref2.invalidate_all();
    myPool.remove_unreferenced_items();
    ...
    ref1.invalidate_all(); // release element 0 (at the start of the pool)
    myPool.remove_unreferenced_items();
    ...
    ref4.invalidate_all(); // release element 0 (at the start of the pool)
    myPool.remove_unreferenced_items();
...
TEST_F(weak_ref_data_pool, remove_unreferenced_items_all_at_once) {
  common::reset();
...

  {
    ...
    ASSERT_EQ(common::getTotalCtor(), 0);
    ASSERT_EQ(common::getTotalCopyCtor(), 0);
    ASSERT_EQ(common::getTotalMoveCtor(), 9);
